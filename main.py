...
async def raw_text_search_handler(message):
    if not await check_authorization(message) or not await handle_anti_spam(message):
        return
    ...
    langs = load_languages()
    if str(user_id) not in langs:
        msg = await bot.send_message(
            chat_id=message.chat.id,
            text="🌸 *Welcome to Movies Paglu!* 🥺🍿\n\nPlease select your language first to unlock live search features!",
            reply_markup=get_lang_keyboard(),
            parse_mode="Markdown"
        )
        asyncio.create_task(delayed_delete(msg.chat.id, msg.message_id, 15))
        return
        
    query = message.text.strip()
    total_queries_handled += 1
    track_user(message.from_user, f"Raw Direct Search: '{query}'")
